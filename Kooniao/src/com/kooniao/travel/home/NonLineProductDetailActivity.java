...
public class NonLineProductDetailActivity extends BaseFragmentActivity {
	...
	private void initData() {
		if (progressDialog == null) {
			progressDialog = new KooniaoProgressDialog(NonLineProductDetailActivity.this);
		}
	...
	private void initTitleTab() {
		titleBarTab.setTabTwoVisibility(View.GONE);
		titleBarTab.setTabOneText(R.string.detail);
	}
	...
	void loadProductDetailComplete(String errMsg, ProductDetail productDetail) {
		...
		if (isNeedToInit) {
			if (errMsg == null) {
				...
				Toast.makeText(NonLineProductDetailActivity.this, errMsg, Toast.LENGTH_SHORT).show();
			}
		}
	}
	...
	private void initView() {
		...
		// Intent storeIntent = new Intent(NonLineProductDetailActivity.this,
	 ...
	 * 详情tab点击
	 */
	void onDetailTabClick() {
		switchFragment(productDetailFragment);
	}
	...
	private ProductDetailFragment_ productDetailFragment; // 详情
	...
	private void initFragment() {
		...
		getSupportFragmentManager().beginTransaction().replace(R.id.fl_fragment_container, productDetailFragment).commit();
	}
	...
	void onSetUpStoreClick() {
		...
		if (localUserInfo == null) {
			// 用户没有登录
			Intent intent = new Intent(NonLineProductDetailActivity.this, LoginActivity_.class);
	...
	void openStore(UserInfo localUserInfo) {
		if (localUserInfo != null) {
			...
			if (shopC == 0) {
				// 用户没开过C店
				int storeId = productDetail.getShopId();
				intent = new Intent(NonLineProductDetailActivity.this, OpenStoreActivity_.class);
				intent.putExtra(Define.SID, storeId);
				intent.putExtra(Define.STORE_TYPE, storeType);
			} else {
				// 用户已经开过C店
				intent = new Intent(NonLineProductDetailActivity.this, BottomTabBarActivity_.class);
	...
	void onShareClick() {
		...
		onekeyShare.show(NonLineProductDetailActivity.this);
	}
	...
	void onCollectClick() {
		...
		if (uid == 0) {
			Intent logIntent = new Intent(NonLineProductDetailActivity.this, LoginActivity_.class);
	...
	void onCustomerServiceClick() {
		...
		if (!"".equals(mobile)) {
			dialog = new Dialog(NonLineProductDetailActivity.this, dialogTitle, mobile);
			...
			Toast.makeText(NonLineProductDetailActivity.this, R.string.customer_service_null, Toast.LENGTH_SHORT).show();
		}
	}
	...
	private void bookingProduct() {
		Intent productBookingIntent = new Intent(NonLineProductDetailActivity.this, SelectProductPackageActivity_.class);
	...
	void addOrCancelToMyCollectComplete(int isKeep, String errMsg) {
		if (errMsg == null) {
			...
			if (isKeep == 1) {
				collectImageView.setImageResource(R.drawable.collect_blue_press);
				Toast.makeText(NonLineProductDetailActivity.this, addToMyCollectSuccessTips, Toast.LENGTH_SHORT).show();
			} else {
				collectImageView.setImageResource(R.drawable.collect_blue_normal);
				Toast.makeText(NonLineProductDetailActivity.this, cancelMyCollectSuccessTips, Toast.LENGTH_SHORT).show();
			}
		} else {
			if (isKeep == 1) {
				collectImageView.setImageResource(R.drawable.collect_blue_normal);
	...
	final int REQUEST_CODE_LOGIN_ADD_TO_MY_COLLECT = 111; // 收藏请求登录
	final int REQUEST_CODE_OPEN_STORE_LOGIN = 113; // 开店请求登录

	@Override
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		switch (requestCode) {
		case REQUEST_CODE_LOGIN_ADD_TO_MY_COLLECT: // 收藏
	...
	void loadTravelLargeImageListComplete(String errMsg, List<String> imgList) {
		if (errMsg == null) {
			if (imgList.isEmpty()) {
				...
				Intent intent = new Intent(NonLineProductDetailActivity.this, ImageBrowseActivity_.class);
	...
	void onShopbSelectClick() {
		Intent storeIntent = new Intent(NonLineProductDetailActivity.this, StoreActivity_.class);
		// 当前店铺id和类型
		int storeId = productDetail.getShopId();
		storeIntent.putExtra(Define.SID, storeId);
		String currentStoreType = "a";
