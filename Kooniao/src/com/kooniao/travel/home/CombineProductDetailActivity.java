...
public class CombineProductDetailActivity extends BaseFragmentActivity {
	...
	private void initData() {
		if (progressDialog == null) {
			progressDialog = new KooniaoProgressDialog(CombineProductDetailActivity.this);
		}
	...
	private void initTitleTab() {
		titleBarTab.setTabOneText(R.string.product);
		titleBarTab.setTabTwoText(R.string.detail);
	}
	...
	void loadProductDetailComplete(String errMsg, ProductDetail productDetail) {
		...
		if (isNeedToInit) {
			if (errMsg == null) {
				...
				Toast.makeText(CombineProductDetailActivity.this, errMsg, Toast.LENGTH_SHORT).show();
			}
		}
	}
	...
	private void initView() {
		...
		final int fragmentMinHeight = Define.heightPx - titleBarTab.getHeight() * 2;
			...
			public void onLayoutChange(View v, int left, int top, int right, int bottom, int oldLeft, int oldTop, int oldRight, int oldBottom) {
				if (bottom - top > fragmentMinHeight) {
					runOnUiThread(new Runnable() {
						...
						@Override
						public void run() {
							shopcenterlayout.setVisibility(View.INVISIBLE);
...
//			productBookingImageView.setImageResource(R.drawable.product_booking_unable);
	 ...
	 * 组合产品tab点击
	 */
	void onCombineProductTabClick() {
		switchFragment(combineProductListFragment);
	}
	...
	private CombineProductListFragment_ combineProductListFragment; // 线路
	private ProductIntroFragment_ productIntroFragment; // 组合产品介绍
	...
	private void initFragment() {
		combineProductListFragment = new CombineProductListFragment_();
		productIntroFragment = new ProductIntroFragment_();
		productCommentFragment = new ProductCommentFragment_();
		/**
		 * 默认选中组合产品tab
		 */
		currentFragment = combineProductListFragment;

		// 把产品详情设置给combineProductListFragment
		Bundle lineData = new Bundle();
		lineData.putSerializable(Define.DATA, productDetail);
		combineProductListFragment.setArguments(lineData);

		// 把组合介绍设置给bookingInfomationFragment
		...
		getSupportFragmentManager().beginTransaction().replace(R.id.fl_fragment_container, combineProductListFragment).commit();
	}
	...
	void onSetUpStoreClick() {
		...
		if (localUserInfo == null) {
			// 用户没有登录
			Intent intent = new Intent(CombineProductDetailActivity.this, LoginActivity_.class);
	...
	void openStore(UserInfo localUserInfo) {
		if (localUserInfo != null) {
			...
			if (shopC == 0) {
				...
				intent = new Intent(CombineProductDetailActivity.this, OpenStoreActivity_.class);
				intent.putExtra(Define.SID, storeId);
				intent.putExtra(Define.STORE_TYPE, storeType);
			} else {
				// 用户已经开过C店
				intent = new Intent(CombineProductDetailActivity.this, BottomTabBarActivity_.class);
	...
	void onShareClick() {
		...
		onekeyShare.show(CombineProductDetailActivity.this);
	}
	...
	void onCollectClick() {
		...
		if (uid == 0) {
			Intent logIntent = new Intent(CombineProductDetailActivity.this, LoginActivity_.class);
			startActivityForResult(logIntent, REQUEST_CODE_LOGIN_ADD_TO_MY_COLLECT);
		} else {
	...
	void onCustomerServiceClick() {
		...
		if (!"".equals(mobile)) {
			dialog = new Dialog(CombineProductDetailActivity.this, dialogTitle, mobile);
			...
			Toast.makeText(CombineProductDetailActivity.this, R.string.customer_service_null, Toast.LENGTH_SHORT).show();
		}
	}
	...
	private void bookingProduct() {
		Intent productBookingIntent = new Intent(CombineProductDetailActivity.this, SelectProductPackageActivity_.class);
	...
	void addOrCancelToMyCollectComplete(int isKeep, String errMsg) {
		if (errMsg == null) {
			...
			if (isKeep == 1) {
				collectImageView.setImageResource(R.drawable.collect_blue_press);
				Toast.makeText(CombineProductDetailActivity.this, addToMyCollectSuccessTips, Toast.LENGTH_SHORT).show();
			} else {
				collectImageView.setImageResource(R.drawable.collect_blue_normal);
				Toast.makeText(CombineProductDetailActivity.this, cancelMyCollectSuccessTips, Toast.LENGTH_SHORT).show();
			}
		} else {
			if (isKeep == 1) {
				collectImageView.setImageResource(R.drawable.collect_blue_normal);
	...
	final int REQUEST_CODE_LOGIN_ADD_TO_MY_COLLECT = 111; // 登录
	final int REQUEST_CODE_OPEN_STORE_LOGIN = 112; // 开店请求登录

	@Override
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		switch (requestCode) {
		case REQUEST_CODE_LOGIN_ADD_TO_MY_COLLECT: // 登录
	...
	void loadTravelLargeImageListComplete(String errMsg, List<String> imgList) {
		if (errMsg == null) {
			Intent intent = new Intent(CombineProductDetailActivity.this, ImageBrowseActivity_.class);
	 ...
	 * 点击店铺名
	 */
	@Click(R.id.tv_product_resource)
	void onStoreNameClick() {
		Intent storeIntent = new Intent(CombineProductDetailActivity.this, StoreActivity_.class);
		// 当前店铺id和类型
		int storeId = productDetail.getShopId();
