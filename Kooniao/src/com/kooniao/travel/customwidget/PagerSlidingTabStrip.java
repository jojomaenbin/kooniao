...
public class PagerSlidingTabStrip extends HorizontalScrollView {
	...
	private int indicatorColor = 0xFF16B8EB; // 底部跟随文字线条颜色
	private int underlineColor = 0x1A000000; // 底部分割线条颜色
	private int dividerColor = 0x5ACD1719; // 分割线颜色

	private boolean shouldExpand = false;
	private boolean textAllCaps = true;

	private int scrollOffset = 52;
	private int indicatorHeight = 2; // 底部跟随线条的高度
	private int underlineHeight = 2;
	private int dividerPadding = 12;
	private int tabPadding = 24;
	private int dividerWidth = 1;

	private int tabTextSize = (int) (8 * Define.DENSITY); // tab的字体大小
	...
	public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
		...
		if (dm.widthPixels > 720) {
			tabTextSize = (int) (6 * Define.DENSITY);
		}
	...
	private void addTextTab(final int position, String title) {
		...
		tab.setSingleLine();
		tab.setWidth(Define.widthPx / 4); // 设置tab的宽度
	...
	private void updateTabStyles() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (v instanceof TextView) {
				...
				if (i == 0) {
					tab.setTextColor(getResources().getColor(R.color.v16b8eb));
				} else {
		...
		public void onPageSelected(int position) {
			for (int i = 0; i < tabCount; i++) {
				...
				if (view instanceof TextView) {
					...
					if (i == position) {
						selectedTextView.setTextColor(getResources().getColor(R.color.v16b8eb));
					} else {
						selectedTextView.setTextColor(tabTextColor);
					}
				}
			}
