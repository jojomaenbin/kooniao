...
public class EditPage extends FakeActivity implements OnClickListener, TextWatcher {
	...
	private TitleLayout getPageTitle() {
		...
		int resIdShare = getStringRes(activity, "multi_share");
		if (resIdShare > 0) {
			llTitle.getTvTitle().setText(resIdShare);
			llTitle.getTvTitle().setGravity(Gravity.CENTER);
		}
		llTitle.getBtnRight().setVisibility(View.VISIBLE);
		resIdShare = getStringRes(activity, "share");
		if (resIdShare > 0) {
			llTitle.getBtnRight().setText(resIdShare);
		}
